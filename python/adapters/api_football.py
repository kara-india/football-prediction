...
class APIFootballAdapter:
    def __init__(
        ...
        # Do not hide retries inside the HTTP adapter. Every outbound attempt
        # must have its own explicit centralized quota reservation.
    ...
    def _make_request(
        ...
        if not self.api_key:
            raise APIError(
                "API_FOOTBALL_KEY is not configured; refusing external API call."
            )

        # Reserve exactly once for this outbound HTTP attempt.
        self.quota_manager.reserve(is_user=is_user, cost=1)

        url = f"{self.base_url}/{endpoint}"
        self.logger.info("Request: %s %s", url, params)
    ...
    def health_check(self) -> dict:
        return self._make_request("status", ttl=0)
    ...
    def get_fixture(self, fixture_id: int) -> Dict:
        ...
        result = self._make_request("fixtures", params, ttl=300)
        if not result:
    ...
    def get_live_fixtures(self, league_ids: List[int]) -> List[Dict]:
        ...
        return self._make_request(
            "fixtures", {"live": "-".join(map(str, league_ids))}, ttl=30
        )

    def get_fixture_lineups(self, fixture_id: int) -> List[Dict]:
        return self._make_request(
            "fixtures/lineups", {"fixture": fixture_id}, ttl=600
        )

    def get_fixture_events(self, fixture_id: int) -> List[Dict]:
        return self._make_request(
            "fixtures/events", {"fixture": fixture_id}, ttl=300
        )

    def get_fixture_statistics(self, fixture_id: int) -> Dict:
        result = self._make_request(
            "fixtures/statistics", {"fixture": fixture_id}, ttl=300
        )
        return result[0] if result else {}

    def get_teams(self, league_id: int, season: int) -> List[Dict]:
        return self._make_request(
            "teams", {"league": league_id, "season": season}, ttl=86400
        )

    def get_team(self, team_id: int) -> Dict:
        result = self._make_request("teams", {"id": team_id}, ttl=86400)
        return result[0] if result else {}
    ...
    def get_player_statistics(
        self, player_id: int, season: int, league_id: int = None
    ) -> Dict:
        params = {"id": player_id, "season": season}
        if league_id:
            params["league"] = league_id
        result = self._make_request("players", params, ttl=3600)
        return result[0] if result else {}
    ...
    def get_standings(self, league_id: int, season: int) -> List[Dict]:
        return self._make_request(
            "standings", {"league": league_id, "season": season}, ttl=1800
        )
    ...
    def get_odds(
        self, fixture_id: int = None, league_id: int = None, bookmaker_id: int = 6
    ) -> List[Dict]:
